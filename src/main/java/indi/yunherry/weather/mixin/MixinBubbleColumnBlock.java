    ...
    public void weather$tick(ClientLevel level, TickBlockInfo info) {
        ...
        if (randomSource.nextInt(10) == 0) {
            ...
            for(int i = 0; i < randomSource.nextInt(1) + 1; ++i) {
                weather$generateMagmaWaterParticles(info.state(), level,info.pos(),randomSource);
            }
        }

    }
    @Unique
    private void weather$generateMagmaWaterParticles(BlockState state, Level level, BlockPos pos, ThreadLocalRandom random) {
